...
يحتوي على بيانات الموظفين المسجّلين في نظام بوت واتساب.

| العمود | النوع | الوصف |
|--------|-------|-------|
| `id` | `serial` (PK) | المعرّف التسلسلي |
| `fullName` | `text` | الاسم الكامل للموظف |
| `phoneNumber` | `text` | UNIQUE — رقم الهاتف |
| `whatsappLid` | `text` | معرّف الجهاز الفريد (WhatsApp LID) |
...
activationCode + deactivationCode يُعيَّنان يدوياً
         ↓
whatsappLid = null (لم يُسجَّل بعد)
         ↓
الموظف يُرسل activationCode عبر واتساب
         ↓
isBotActive = true، whatsappLid يُسجَّل تلقائياً
         ↓
...
**حقل `whatsappLid`:**
- يُملأ تلقائياً من رسالة التفعيل الأولى
- يُستخدم لمنع سرقة الجلسة
- يمكن مسحه (إعادة تعيين) من لوحة التحكم لتغيير الجهاز
